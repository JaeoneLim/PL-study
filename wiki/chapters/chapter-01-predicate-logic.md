...
- 구문: 어떤 종류의 구절이 있으며 어떤 생성자로 만들어지는가?
- 의미: 각 구절은 어떤 수학적 대상을 나타내는가?
- 증명: 어떤 형식의 판단을 어떤 규칙으로 유도할 수 있는가?
- 바인딩: 이름을 묶는 구문은 범위·자유 변수·치환을 어떻게 바꾸는가?
...
## 02. 문자열과 구문 구조를 분리하기 — §1.1 · pp. 1–3 · 6분
...
> [!example] 표면 표기에서 생성자 트리로
> `x + y × z < 10 ∧ ¬(x = 0)`을 구조로 읽어 보자.
>
> 1. 곱셈이 덧셈보다 먼저 묶이므로 왼쪽 정수식은 `x + (y × z)`다.
> 2. 관계 연산 `<`가 두 정수식을 받아 단언 `x + (y × z) < 10`을 만든다.
> 3. `¬`가 등식 단언 `x = 0`을 감싸고, 마지막으로 `∧`가 두 단언을 결합한다.
> 4. 생성자식으로는 대략 `C∧(C<(C+(Cvar(x), C×(Cvar(y), Cvar(z))), C10), C¬(C=(Cvar(x), C0)))`이다.
>
> **결론:** 이 트리가 확정된 뒤에는 중위 표기나 우선순위가 달라져도 같은 추상 구문을 가리킬 수 있다.
...
> [!question] 이 절을 덮고 확인하기
> 1. `¬x = 0 ∨ y < 3`에 필요한 괄호를 모두 복원하고 가장 바깥 생성자를 말해 보라.
> 2. 연산자 우선순위가 의미론이 아니라 구체 구문의 문제인 이유를 설명하라.
...
## 03. 운반집합·생성자·초기 대수 — §1.1 · pp. 3–8 · 7분
...
### 올바른 추상 구문 구현이 만족해야 할 세 조건

- 단사성: 같은 생성자의 결과가 같으면 각 인자도 같아야 한다. `C+(e₀,e₁)=C+(e₂,e₃)`이면 `e₀=e₂`, `e₁=e₃`이다.
- 서로소 치역: 같은 운반집합으로 들어가는 서로 다른 생성자의 결과는 겹치지 않는다. 덧셈 구절이 동시에 곱셈 구절일 수 없다.
- 유한 생성: 미리 주어진 집합의 원소를 제외한 모든 구절은 생성자를 유한 번 적용해 만들어져야 한다. 무한 깊이의 유령 구절은 허용하지 않는다.
...
> [!example] 하나의 추상 구문, 여러 실현
> 구조 `(0 - 1) + (-2)`를 세 방식으로 실현해 보자.
>
> 1. 완전 괄호 중위 문자열: `((0) - (1)) + (-(2))`
> 2. 접두 함수 문자열: `add(subtract(0,1), negate(2))`
> 3. 구문 트리: 루트 `+`, 왼쪽 자식은 루트 `-binary`와 잎 0·1, 오른쪽 자식은 루트 `-unary`와 잎 2
> 4. 대수적 자료형 값: `Plus(Sub(Const(0), Const(1)), Neg(Const(2)))`
>
> **결론:** 표현은 다르지만 각 생성자의 단사성, 서로소 치역, 유한 생성이라는 같은 계약을 만족하므로 이후 의미 정의와 증명을 공유한다.
...
> [!question] 이 절을 덮고 확인하기
> 1. 생성자의 단사성과 서로소 치역이 각각 어떤 종류의 모호성을 막는지 구분해서 설명하라.
...
>
> **결론:** 계산은 언제나 최상위 생성자의 방정식을 선택하고 직접 부분구절로 내려간 뒤, 얻은 의미를 다시 조합하는 방식으로 진행된다.
...
> 2. 닫힌 단언의 의미가 상태와 무관해야 하는 이유를 직관적으로 설명하라.
> 3. 정수식과 단언의 의미 함수 공역이 왜 다른가?
...
## 05. 구문 지향성, 유일성, 합성성 — §1.2 · pp. 9–12 · 6분
...
> 2. 따라서 문맥 `[-]×y<z`에 넣은 `(x+0)×y<z`와 `x×y<z`도 같은 단언 의미를 갖는다.
...
> [!question] 이 절을 덮고 확인하기
> 1. 구문 지향 의미 방정식의 두 조건을 말하고, 각각이 빠졌을 때 생길 문제를 설명하라.
...
> 4. 마지막으로 타당한 단언을 전칭 일반화하는 규칙을 적용해 `∀x. x=x+0`을 얻을 수 있다.
>
> **결론:** 스키마는 무한히 많은 규칙 인스턴스를 압축하고, 증명은 그 인스턴스들을 유한 번 연결한다.

> [!tip] 건전성은 구문과 의미 사이의 다리
...
> 단언 `∀x. (x<y ∨ ∃y. (x=y ∧ ∀x. x>y))`를 바깥에서 안쪽으로 읽는다.
>
...
> 3. 일치 정리로 닫힌 단언의 상태 독립성을 한 줄로 유도하라.
...
> 3. 원래 의도는 현재 상태의 `y+1`보다 큰 정수가 존재한다는 말이어서 항상 참이지만, 포획된 결과는 어떤 정수도 자기보다 1 클 수 없다는 거짓 단언이다.
...
>
> 1. 구문 결과는 `(y+1)+2<y`다. 원래 `y`는 치환 대상이 아니므로 그대로 남는다.
...
> 3. 치환 정리의 양변에서 구문 조작과 상태 조작이 각각 어디에 나타나는가?
...
> [!example] 연습 1 — 자연어를 수량자 구조로
> ‘0보다 크고 4보다 작은 서로 다른 정수가 적어도 두 개 있다’를 이 장의 단언으로 적어 보라.
>
...
> [!example] 연습 4 — 새 바인딩 구문 설계
> 합 `sum v from e₀ to e₁ of e₂`를 정수식으로 추가한다고 하자. 어떤 항목을 정의해야 하는가?
>
> 1. 추상 구문: `v`, 아래·위 경계식 `e₀,e₁`, 본문 `e₂`를 받는 생성자를 추가한다. `v`는 본문에서만 바인더이고 경계식에서는 자유로 읽는 설계를 명시한다.
...
> 5. 그 뒤 일치 정리와 치환 정리의 새 생성자 경우를 증명해야 언어 확장이 기존 메타이론과 호환된다.
>
...
- 구문적 동일성과 알파 동치를 구분했다.

> [!question] 이 절을 덮고 확인하기
> 1. 위 연습을 보지 않고 약수와 소수의 단언을 다시 구성하라.
> 2. 새 바인딩 구문을 추가할 때 함께 확장해야 하는 다섯 요소를 나열하라.
...
- 먼저 표면 문자열에서 독립된 구절 종류와 생성자를 정한다. 단사성·서로소 치역·유한 생성이 재귀와 귀납의 토대를 만든다.
- 각 구절 종류에 알맞은 의미 영역을 고르고, 모든 생성자에 구문 지향 의미 방정식을 하나씩 준다.
- 구문 지향성과 초기성으로 의미 함수의 존재·유일성과 합성성을 얻는다.
- 의미론으로 상태에서의 참과 타당성을 정의하고, 추론 규칙의 건전성으로 증명 가능성을 의미적 참에 연결한다.
...
> [!question] 이 절을 덮고 확인하기
> 1. 처음 보는 `let v=e in p` 구문을 추가한다면 1장의 순서에 따라 무엇부터 무엇까지 정의할지 설계안을 써 보라.
