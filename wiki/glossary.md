...
**English:** The set containing every possible value of one syntactic or semantic sort. In Chapter 1, `IntExp` and `Assert` are carrier sets for integer expressions and assertions.
...
**English:** A structure containing exactly the finite syntax generated by its constructors, with no extra equalities. Once every constructor is interpreted in a target algebra, exactly one homomorphism to that target is determined.
