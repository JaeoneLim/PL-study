...
export const chapterGuides: Record<string, ChapterGuide> = {
  "predicate-logic": {
    purpose: b("이후 모든 장에서 사용할 세 가지 층위—구문, 의미, 증명—를 분리하고 다시 연결하는 법을 세운다.", "Establishes how to separate and reconnect the three layers used throughout the book: syntax, semantics, and proof."),
    sections: [
      section("§1.1", "추상 구문과 구조적 정의", "Abstract syntax and structural definition", "논리식의 표면 표기 대신 항과 공식의 생성자를 정의한다. 이 구조는 재귀 함수와 구조적 귀납법의 기준이 된다.", "Defines constructors for terms and formulas independently of surface notation. That structure supports recursive functions and structural induction."),
      section("§1.2", "환경을 통한 표시적 의미", "Denotational meaning through environments", "변수 환경과 기호 해석을 입력으로 받아 항은 값으로, 공식은 진릿값으로 보낸다. 의미 함수의 각 절은 구문 생성자에 대응한다.", "Given a variable environment and an interpretation of symbols, terms denote values and formulas denote truth values. Each semantic clause follows a syntax constructor."),
      ...
      section("§1.4", "바인딩, 이름 변경, 치환", "Binding, renaming, and substitution", "자유·결박 변수를 정의하고 캡처 회피 치환을 구성한다. 대입 보조정리는 구문 치환과 환경 갱신이 같은 의미 효과를 낸다는 연결 고리다.", "Defines free and bound variables and constructs capture-avoiding substitution. The substitution lemma connects syntactic replacement with semantic environment update."),
    ],
    takeaways: [
      b("추상 구문은 파싱 결과이며, 의미와 증명은 그 트리 위에서 재귀적으로 정의된다.", "Abstract syntax is the result of parsing; semantics and proofs are recursively defined over that tree."),
  ...
  "simple-imperative-language": {
    ...
    sections: [
      section("§2.1–2.2", "구문, 상태, 의미 함수", "Syntax, states, and semantic functions", "식은 상태에서 값으로, 명령은 상태에서 상태로 가는 부분 함수로 해석된다. 정의되지 않음은 산술 오류가 아니라 비종료를 먼저 나타낸다.", "Expressions map states to values, while commands are partial functions from states to states. Undefinedness initially represents nontermination rather than arithmetic failure."),
  ...
  "program-specifications": {
    ...
    sections: [
      ...
      section("§3.8", "복잡성, 종료, 표현력의 한계", "Complications, termination, and limits", "부분 정확성과 전체 정확성을 구분하고, 종료 함수와 단언 언어의 표현력이 추가로 필요함을 확인한다. 규칙의 건전성과 상대적 완전성도 별개의 질문이다.", "Separates partial from total correctness and identifies the extra need for variants and an expressive assertion language. Soundness and relative completeness of the rules are distinct questions."),
    ],
    takeaways: [
  ...
  arrays: {
    purpose: b("배열을 포함하도록 상태와 단언 논리를 확장하고, 이진 탐색을 통해 데이터 구조 불변식과 범위 추론을 연습한다.", "Extends states and assertion logic with arrays, using binary search to practice data-structure invariants and range reasoning."),
    sections: [
      section("§4.1–4.2", "배열 구문과 함수적 저장", "Array syntax and functional stores", "배열 읽기와 갱신을 핵심 구문에 추가하고, 배열 값을 인덱스에서 원소로 가는 유한 함수로 본다. 갱신은 새 배열 값을 만들되 나머지 인덱스를 보존한다.", "Adds array selection and update, treating an array value as a finite function from indices to elements. Update creates a new array value while preserving all other indices."),
      ...
      section("§4.4", "배열 대입의 추론 규칙", "Inference rules for array assignment", "스칼라 대입의 단순 치환 대신 배열 갱신 식을 단언에 반영한다. 선택-갱신 관계는 같은 인덱스와 다른 인덱스의 경우를 나눈다.", "Replaces scalar substitution with array-update expressions inside assertions. Selection after update splits into equal-index and unequal-index cases."),
      section("§4.5", "배열 전체에 대한 고차 단언", "Higher-order assertions about arrays", "정렬됨, 구간 내 일치, 모든 원소의 성질처럼 인덱스를 양화하는 단언을 사용한다. 단언 언어가 배열 함수와 양화를 충분히 표현해야 증명 규칙이 유용하다.", "Uses quantified assertions for sortedness, interval agreement, and elementwise properties. Useful proof rules require an assertion language expressive enough for arrays and quantification."),
    ],
    takeaways: [
      ...
      b("데이터 구조가 풍부해지면 단언 언어의 표현력도 함께 확장되어야 한다.", "As data structures grow richer, the assertion language must grow with them."),
    ],
    cautions: [
  ...
  "failure-io-continuations": {
    ...
    sections: [
      ...
      section("§5.7", "계속 의미론", "Continuation semantics", "명령의 의미가 최종 결과가 아니라 ‘나머지 계산을 받아 전체 결과를 만드는 함수’가 된다. 순차 합성은 명령들이 같은 계속을 넘겨받는 방식으로 단순해진다.", "A command denotes not a final result but a function that accepts the rest of the computation and produces the whole result. Sequencing becomes continuation threading."),
    ...
    takeaways: [
      b("관찰 가능한 효과가 늘 때마다 단순 상태 변환 의미론의 결과형이 부족해진다.", "Each new observable effect exposes missing structure in plain state-transformer semantics."),
  ...
  "transition-semantics": {
    ...
    sections: [
      ...
      section("§6.2", "구조적 실행 규칙", "Structural execution rules", "대입, 순차 합성, 조건, 반복의 실행을 추론 규칙으로 정의한다. 복합 명령의 단계는 직접 하위 명령의 단계에서 유도되므로 구문 구조를 따른다.", "Inference rules define execution of assignment, sequencing, conditionals, and loops. Steps of a compound command are derived from steps of its immediate subcommands."),
    ...
    takeaways: [
      b("표시적 의미론은 전체 결과를, 전이 의미론은 결과가 만들어지는 과정을 강조한다.", "Denotational semantics emphasizes whole results; transition semantics emphasizes the process that produces them."),
  ...
  nondeterminism: {
    ...
    sections: [
      section("§7.1", "가드 명령과 선택", "Guarded commands and choice", "여러 참 가드 중 하나를 선택해 실행하는 조건·반복 구문을 전이 규칙으로 정의한다. 선택 방법은 언어 의미가 의도적으로 정하지 않는다.", "Transition rules define conditionals and loops that choose one enabled guard. The language semantics deliberately leaves the choice unresolved."),
      ...
      section("§7.5", "최약 전제조건", "Weakest preconditions", "wp(C,Q)는 C가 원하는 종료 의미 아래 Q를 보장하게 하는 가장 약한 시작 조건이다. 구문별 변환 법칙은 프로그램을 논리식으로 역계산한다.", "wp(C,Q) is the weakest starting condition ensuring that C establishes Q under the chosen termination interpretation. Syntax-directed equations calculate it backward."),
    ],
    takeaways: [
      ...
      b("최약 전제조건은 의미론과 자동 검증 조건 생성 사이의 다리다.", "Weakest preconditions bridge semantics and automatic verification-condition generation."),
    ],
    cautions: [
  ...
  "shared-variable-concurrency": {
    ...
    takeaways: [
      ...
      b("공정성은 프로그램 코드가 아니라 허용할 스케줄에 대한 의미론적 가정이다.", "Fairness is a semantic assumption about admitted schedules, not merely a property of program text."),
    ],
    cautions: [
  ...
  "eager-functional-language": {
    ...
    sections: [
      ...
      section("§11.4–11.5", "목록과 함수형 예제", "Lists and functional examples", "목록 생성자와 패턴을 이용해 map, fold류 재귀 처리와 정렬·탐색 예제를 표현한다. 데이터 구조 재귀와 함수 재귀의 모양이 맞물린다.", "List constructors and patterns express recursive processing such as map- and fold-like functions, sorting, and search. Data recursion aligns with function recursion."),
  ...
  "functional-continuations": {
    ...
    sections: [
      section("§12.1", "계속 전달 의미론", "Continuation semantics", "식의 의미는 값 하나가 아니라 결과를 받아 다음 계산을 수행할 계속을 인수로 받는다. 평가 순서는 계속이 중첩되는 순서에 명시적으로 나타난다.", "An expression meaning accepts a continuation that consumes its value and performs the rest of the computation. Evaluation order becomes explicit in continuation nesting."),
      ...
      section("§12.6", "두 의미론의 대응", "Relating the two semantics", "일차 프레임을 다시 계속 함수로 해석하는 대응을 정의하고 각 기계 단계가 계속 의미를 보존함을 보여 두 설명의 동치를 정당화한다.", "Interprets first-order frames back as continuation functions and shows each machine step preserves continuation meaning, justifying equivalence of the accounts."),
    ],
    takeaways: [
  ...
  "iswim-like-languages": {
    ...
    sections: [
      ...
      section("§13.2–13.3", "상태를 통과시키는 평가와 계속", "State-threaded evaluation and continuations", "평가 판단은 값과 함께 갱신된 저장소를 반환한다. 계속 의미론에서는 계속이 값과 저장소를 함께 받아 평가 순서와 부작용 순서를 동시에 고정한다.", "Evaluation returns an updated store along with a value. In continuation semantics, continuations consume both value and store, fixing evaluation order and effect order together."),
      section("§13.4–13.6", "문법 설탕, 일차 기계, 예제", "Derived forms, first-order machine, and examples", "대입·블록·반복 같은 편의 구문을 핵심 참조 연산으로 번역하고, 계속을 비함수화해 환경-저장소-제어 스택 기반 평가기를 얻는다.", "Translates assignment, blocks, loops, and other conveniences into core reference operations, then defunctionalizes continuations into an evaluator with environment, store, and control stack."),
  ...
  "normal-order-language": {
    ...
    sections: [
      ...
      section("§14.2–14.3", "파생 구문과 무한 구조 예제", "Derived forms and infinite structures", "조건, 목록, 재귀를 정상 순서 핵심으로 표현하면 사용하지 않는 분기를 피하고 잠재적으로 무한한 목록의 유한 접두사를 소비할 수 있다.", "Encoding conditionals, lists, and recursion in the normal-order core avoids unused branches and permits finite consumption of potentially infinite lists."),
  ...
  "simple-type-system": {
    ...
    sections: [
      section("§15.1–15.2", "타입, 문맥, 판단, 규칙", "Types, contexts, judgments, and rules", "Γ ⊢ e : τ는 문맥 Γ가 자유 변수 타입을 가정할 때 e가 τ형이라는 유도 가능한 주장이다. 변수·함수·적용·곱·합 규칙이 구문 구조를 따른다.", "Γ ⊢ e : τ is a derivable claim that e has type τ under assumptions Γ for free variables. Rules for variables, functions, application, products, and sums follow syntax structure."),
      section("§15.3", "명시적 타입 표기", "Explicit typing", "람다 매개변수와 재귀 지점에 타입을 기록하면 타입 검사 규칙의 선택이 구문에 드러난다. 암시적 추론과 달리 유도 탐색의 모호성이 줄어든다.", "Annotating lambda parameters and recursive points exposes typing choices in syntax and reduces ambiguity compared with implicit inference."),
      ...
      section("§15.5", "내재적 관점", "Intrinsic view", "타입 유도 자체를 프로그램 구문의 일부로 보아 잘 타입된 항만 언어에 존재하게 한다. 같은 표면 항도 서로 다른 유도에 따라 다른 내재 객체가 될 수 있다.", "Treats typing derivations as part of program syntax so only well-typed terms inhabit the language. One surface term may correspond to different intrinsic objects through different derivations."),
  ...
  "mathematical-background": {
    ...
    sections: [
      section("§A.1", "집합과 멱집합", "Sets and powersets", "원소, 부분집합, 합집합, 교집합, 차집합, 멱집합을 정의한다. 구문 범주·상태 공간·가능 결과 집합을 읽는 기본 표기다.", "Defines membership, subset, union, intersection, difference, and powerset—the base notation for syntax categories, state spaces, and sets of outcomes."),
    ...
    takeaways: [
      b("의미론 표기는 대부분 집합 사이의 함수와 관계를 정밀하게 조합한 것이다.", "Most semantic notation precisely combines functions and relations between sets."),
