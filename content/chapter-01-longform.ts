...
const notation = (title: Bilingual, source: string, explanation: Bilingual): LessonNotationBlock => ({
  kind: "notation",
  title,
  notation: source,
  explanation,
...
export const predicateLogicLongform: ChapterLongform = {
  ...
  introduction: [
    ...
    b(
      "1장의 목적은 술어 논리 자체를 깊이 공부하는 데만 있지 않다. 익숙한 논리를 실험대로 삼아 언어 이론의 네 도구, 곧 추상 구문·표시적 의미론·추론 규칙·바인딩을 정확히 분리하는 것이 핵심이다. 2장 이후에는 대상 언어만 바뀌고 이 네 질문이 계속 돌아온다. 어떤 구문을 허용하는가, 그 구문은 무엇을 뜻하는가, 어떤 주장을 규칙으로 증명할 수 있는가, 이름의 범위와 치환은 의미를 어떻게 보존하는가를 묻는 습관이 이 장의 진짜 성과다.",
  ...
  sections: [
    {
      ...
      blocks: [
        prose(
          ...
          b(
            "술어 논리는 이 작업에 알맞은 첫 대상이다. 첫째, 정수식과 논리식은 이미 익숙하므로 새 표기보다 설명 방법에 집중할 수 있다. 둘째, 이 장의 논리식은 언제나 정수나 진릿값을 내므로 비종료와 부분 정보라는 어려움을 잠시 미룰 수 있다. 셋째, 3장에서 명령형 프로그램의 사전조건과 사후조건을 바로 이 단언 언어로 적게 된다. 따라서 1장은 예비 지식이면서 동시에 이후 증명 체계의 실제 부품이다.",
          ...
          b(
            "이 책의 용어는 전통 논리학과 조금 다르다. 논리학의 항(term)은 정수식(integer expression), 잘 형성된 공식은 단언(assertion), 변수에 값을 배정하는 assignment는 상태(state)라고 부른다. 이런 명칭은 곧 등장할 프로그래밍 언어와 어휘를 맞추려는 선택이다. 여기서 상태는 프로그램 메모리를 아직 뜻하지 않고, 각 변수 이름에 정수 하나를 대응시키는 함수다.",
        ...
        list(
          ...
          b("구문: 어떤 종류의 구절이 있으며 어떤 생성자로 만들어지는가?", "Syntax: What kinds of phrases exist, and which constructors build them?"),
          b("의미: 각 구절은 어떤 수학적 대상을 나타내는가?", "Semantics: What mathematical object does each phrase denote?"),
          b("증명: 어떤 형식의 판단을 어떤 규칙으로 유도할 수 있는가?", "Proof: What judgments can be derived, and by which rules?"),
          b("바인딩: 이름을 묶는 구문은 범위·자유 변수·치환을 어떻게 바꾸는가?", "Binding: How do binders affect scope, free variables, and substitution?")
        ),
        callout(
          "key",
          b("읽는 관점", "Reading lens"),
          b(
            "이 장에서 나오는 정의를 논리학 상식의 반복으로 넘기지 말자. 각각은 뒤에서 명령, 함수, 타입, 모듈을 설명할 때 그대로 재사용되는 설계 패턴이다. 특히 ‘구문을 먼저 정하고, 그 구조를 따라 의미 함수를 정의한다’는 순서는 책 전체의 뼈대다.",
    ...
    },
    {
      ...
      covers: "§1.2 · pp. 9–12",
      minutes: 6,
      title: b("구문 지향성, 유일성, 합성성", "Syntax direction, uniqueness, and compositionality"),
      lead: b(
      ...
      blocks: [
        prose(
          b(
            "아무 방정식이나 의미 정의가 되는 것은 아니다. 이 책에서 의미 방정식은 두 조건을 지킨다. 첫째, 추상 문법의 생산 규칙마다 대응하는 방정식이 하나씩 있어야 한다. 둘째, 구성된 구절의 의미는 직접 부분구절의 의미만으로 계산되어야 한다. 변수처럼 미리 주어진 종류는 부분구절 자체를 사용할 수 있다. 이 대응 때문에 어떤 구절을 만나도 최상위 생성자를 보고 적용할 방정식을 바로 고를 수 있다.",
          ...
          b(
            "이런 정의를 구문 지향적(syntax-directed) 또는 준동형적(homomorphic)이라고 한다. 이름은 의미 계산의 재귀 모양이 구문 트리의 모양을 그대로 따른다는 뜻이다. 덧셈 노드에서는 두 자식의 정수 의미를 더하고, 논리곱 노드에서는 두 자식의 진릿값을 논리곱하며, 수량자 노드에서는 본문의 의미를 갱신된 상태들에 적용한다. 구문 구조와 의미 계산 구조가 평행하다.",
        ...
        callout(
          ...
          b(
            "구절 깊이에 대한 귀납법을 생각하자. 깊이 0에는 구절이 없어 자명하다. 깊이 `j+1`의 구절 `p`는 유한 생성 조건 때문에 어떤 생성자로 만들어졌다. 서로소 치역 때문에 그 생성자의 종류가 하나뿐이고, 단사성 때문에 직접 부분구절도 하나의 튜플로 정해진다. 각 부분구절은 더 얕으므로 귀납가정에 따라 의미가 유일하다. 해당 생성자에 대응하는 의미 방정식이 그 의미들을 결합하는 방법을 하나로 정하므로 `p`의 의미도 유일하다.",
          ...
          b(
            "존재성도 같은 논증에서 얻는다. 생성자와 부분구절을 분해할 수 있고, 부분구절 의미가 이미 존재하며, 오른쪽의 메타언어 연산이 값을 돌려주므로 새 의미가 존재한다. 따라서 구문 집합이 초기 대수이고 의미 함수가 목표 대수로 가는 유일한 준동형이라는 대수적 설명과 깊이 귀납 설명은 같은 사실의 두 얼굴이다.",
        ...
        prose(
          b(
            "구문 지향성은 합성성(compositionality)을 낳는다. 합성적 의미론에서는 전체 의미가 직접 부분구절의 의미만으로 정해진다. 부분구절이 얼마나 길었는지, 어떤 변수 이름을 썼는지, 어떤 과정을 거쳐 만들어졌는지처럼 의미 이외의 속성을 엿보지 않는다. 그 결과 어떤 문맥 안의 부분구절을 의미가 같은 다른 구절로 바꾸어도 전체 의미가 변하지 않는다.",
          ...
          b(
            "예를 들어 모든 상태에서 `e`와 `e'`가 같은 정수를 낸다면 `e+1<10`, `∀x. e=x`, `e=e∧p`처럼 `e`를 품은 어떤 올바른 구문 문맥에서도 `e'`로 바꾸어 의미를 보존한다. 이 대체 원리는 최적화의 정확성, 등식 추론, 모듈 구현 교체를 뒷받침한다. 다만 ‘현재 한 상태에서 값이 같다’가 아니라 의미 함수 전체가 같아야 함에 주의하자.",
        ...
        example(
          ...
          [
            ...
            b("따라서 문맥 `[-]×y<z`에 넣은 `(x+0)×y<z`와 `x×y<z`도 같은 단언 의미를 갖는다.", "Putting either expression in the context `[-]×y<z` produces equivalent assertions."),
        ...
        prose(
          b(
            "의미 방정식에는 대상 언어와 메타언어가 동시에 등장한다. `⟦e₀+e₁⟧σ = ⟦e₀⟧σ + ⟦e₁⟧σ`에서 괄호 안의 `+`는 정의할 언어의 생성자이고, 괄호 밖의 `+`는 이미 알고 있는 정수 덧셈이다. 대상 언어(object language)는 설명 대상, 메타언어(metalanguage)는 설명을 적는 수학 언어다. `e`, `p`, `σ`, `n`은 대상 언어에 실제로 쓰이는 변수가 아니라 구절·상태·정수를 가리키는 메타변수다.",
      ...
      checkpoints: [
        b("구문 지향 의미 방정식의 두 조건을 말하고, 각각이 빠졌을 때 생길 문제를 설명하라.", "State the two requirements on syntax-directed equations and explain what can fail without each."),
    ...
    },
    {
      ...
      lead: b(
        "의미론이 무엇이 참인지 정한다면, 추론 체계는 무엇을 규칙으로 증명할 수 있는지 정한다.",
        "Semantics determines what is true; an inference system determines what can be proved by rules."
      ),
      blocks: [
        prose(
          b(
            "단언 `p`가 한 상태 `σ`에서 참이라는 것은 `⟦p⟧σ=true`라는 뜻이다. ‘`p`가 `σ`를 기술한다’, ‘`σ`가 `p`를 만족한다’도 같은 말이다. 반면 `p`가 타당(valid)하다는 것은 모든 상태에서 참이라는 뜻이다. `x>0`은 어떤 상태에서는 참이지만 타당하지 않다. `x=x`와 `x+0=x`는 모든 상태에서 참이므로 타당하다. 모든 상태에서 거짓인 단언은 만족 불가능(unsatisfiable)하다고 한다.",
          ...
          b(
            "`p₀ ⇒ p₁`이 타당하면 `p₀`를 만족하는 모든 상태가 `p₁`도 만족한다. 이때 `p₀`가 `p₁`보다 강하고, `p₁`이 `p₀`보다 약하다고 한다. 강한 조건일수록 허용하는 상태 집합이 작다. `false`는 아무 상태도 허용하지 않으므로 모든 단언보다 강하고, `true`는 모든 상태를 허용하므로 모든 단언보다 약하다. 두 단언이 서로보다 강하면 같은 상태 집합을 기술하므로 의미적으로 동치다.",
        ...
        notation(
          b("단언을 상태 집합으로 읽기", "Reading assertions as sets of states"),
        ...
        prose(
          b(
            "추론 규칙은 0개 이상의 전제와 하나의 결론으로 이루어진 틀이다. 전제와 결론에는 구체 단언 대신 적절한 구절을 받을 메타변수가 들어갈 수 있다. 메타변수에 실제 대상 언어 구절을 넣으면 규칙의 인스턴스가 된다. 전제가 하나도 없는 규칙은 공리 스키마이고, 메타변수마저 없어 가능한 인스턴스가 하나뿐이면 단순히 공리라고 한다. 부가 조건이 붙은 규칙은 그 조건을 만족하는 교체만 인스턴스로 인정한다.",
          ...
          b(
            "형식 증명은 각 줄이 앞선 줄들을 전제로 하는 어떤 규칙 인스턴스의 결론인 유한한 단언 열이다. 마지막 줄이 증명한 단언이다. 같은 내용을 증명 트리로 그리면 결론이 아래 뿌리에 있고 그 규칙의 전제 증명들이 위쪽 가지가 된다. 트리는 의존 구조를 직접 보여 주지만 큰 증명은 넓어지므로 실제 문서에서는 번호가 매겨진 열과 규칙 주석을 자주 쓴다.",
        ...
        example(
          ...
          [
            ...
            b("마지막으로 타당한 단언을 전칭 일반화하는 규칙을 적용해 `∀x. x=x+0`을 얻을 수 있다.", "Universal generalization of the valid assertion yields `∀x. x=x+0`.")
          ],
          b(
        ...
        callout(
          "key",
          b("건전성은 구문과 의미 사이의 다리", "Soundness bridges syntax and semantics"),
          b(
        ...
        callout(
          ...
          b(
            "함의 `p⇒q`는 한 단언 안의 대상 언어 연결사이며 한 상태에서 진릿값을 갖는다. 추론 `p / q`는 두 증명 단계 사이의 메타언어 관계다. 한 상태에서 `p`가 참이라고 해서 그 상태에서 `∀x.p`도 참인 것은 아니므로 ‘현재 참인 `p`에서 `∀x.p`를 결론 내린다’는 읽기는 틀리다. 전칭 일반화 규칙이 건전한 이유는 전제 `p`가 한 상태에서 참이어서가 아니라 모든 상태에서 타당하다고 가정하기 때문이다.",
        ...
        prose(
          b(
            "완전성은 건전성의 반대 방향이다. 의미적으로 타당한 모든 단언이 규칙으로 증명 가능하면 추론 체계가 완전하다. 여기서는 정수 산술 연산의 의미를 고정했기 때문에 산술적 타당성 전체를 유한 규칙으로 포착할 수 없다는 불완전성 문제가 생긴다. 반면 연산 해석까지 임의로 바꾸어도 참인 논리적 타당성에는 완전한 일차 논리 계산이 존재한다. 프로그램 검증에서는 보통 실제 정수 연산을 고정해야 하므로, 논리 규칙만으로 모든 산술 사실을 해결하겠다는 기대를 버리고 필요한 산술 추론을 별도 이론이나 자동화에 맡긴다.",
    ...
    },
    {
      ...
      covers: "§1.1 · pp. 1–3",
      minutes: 6,
      title: b("문자열과 구문 구조를 분리하기", "Separating strings from syntactic structure"),
      lead: b(
        "프로그램은 문자로 입력되지만 의미가 붙는 대상은 문자 배열이 아니라 파싱된 구문 구조다.",
        "Programs arrive as characters, but semantics is assigned to parsed syntactic structure rather than raw strings."
      ),
      blocks: [
        prose(
          b(
            "자연수 12는 문자 ‘1’과 ‘2’의 연결 그 자체가 아니다. 십진 문자열, 이진 문자열, 로마 숫자는 같은 수를 서로 다르게 나타낸다. 마찬가지로 `x + y`, `add(x,y)`, 루트가 `+`인 트리는 하나의 덧셈 구조를 다른 방식으로 표현한다. 의미 함수를 문자 위치와 괄호 개수에 맞추어 정의하면 표현을 바꿀 때마다 의미론도 다시 써야 한다. 그래서 의미론의 입력은 구체 문자열이 아니라 추상 구절이어야 한다.",
          ...
          b(
            "보통의 문맥 자유 문법은 어떤 문자열이 허용되는지 말하는 데 훌륭하지만, 구문 구조와 표기상의 선택을 함께 적는다. 변수의 철자, 연산자가 앞에 오는지 가운데 오는지, 우선순위와 결합 방향은 파서에는 중요해도 덧셈이라는 구성의 본질은 아니다. 추상 문법은 비단말 기호와 부분구절의 배열로 구조를 드러내면서도, 사람이 읽을 최소한의 표기를 곁들인 절충 형식이다.",
        ...
        notation(
          ...
          b(
            "`var`는 별도 생성 규칙으로 만드는 구절이 아니라 미리 주어진 셀 수 있는 무한 집합이다. 정수 상수도 실제 구현에서는 자연수 하나를 받는 상수 생성자로 묶을 수 있다. 문법의 각 선택지는 뒤에서 하나의 생성자와 하나의 의미 방정식에 대응한다.",
        ...
        prose(
          ...
          b(
            "실제로 글을 쓸 때 모든 부분구절에 괄호를 붙이지는 않는다. 그래서 우선순위와 왼쪽/오른쪽 결합 규칙이 구체 표기의 보조 장치로 필요하다. 하지만 이 규칙은 추상 구문이 아니라 문자열을 트리로 읽기 위한 파싱 약속이다. `x + y × z`가 `x + (y × z)`로 파싱되고 나면 의미론은 곱셈이 높은 우선순위였다는 사실을 더 이상 알 필요가 없다.",
        ...
        example(
          b("표면 표기에서 생성자 트리로", "From surface notation to a constructor tree"),
          ...
          [
            ...
            b("관계 연산 `<`가 두 정수식을 받아 단언 `x + (y × z) < 10`을 만든다.", "The relation `<` consumes two integer expressions and produces an assertion."),
            b("`¬`가 등식 단언 `x = 0`을 감싸고, 마지막으로 `∧`가 두 단언을 결합한다.", "Negation wraps `x = 0`, and conjunction combines the two assertions."),
            b("생성자식으로는 대략 `C∧(C<(C+(Cvar(x), C×(Cvar(y), Cvar(z))), C10), C¬(C=(Cvar(x), C0)))`이다.", "A constructor expression is roughly `C∧(C<(C+(Cvar(x), C×(Cvar(y), Cvar(z))), C10), C¬(C=(Cvar(x), C0)))`.")
          ],
          b(
            "이 트리가 확정된 뒤에는 중위 표기나 우선순위가 달라져도 같은 추상 구문을 가리킬 수 있다.",
      ...
      checkpoints: [
        b("`¬x = 0 ∨ y < 3`에 필요한 괄호를 모두 복원하고 가장 바깥 생성자를 말해 보라.", "Fully parenthesize `¬x = 0 ∨ y < 3` and identify the outermost constructor."),
        b("연산자 우선순위가 의미론이 아니라 구체 구문의 문제인 이유를 설명하라.", "Explain why precedence belongs to concrete syntax rather than semantics.")
      ],
    },
    {
      ...
      covers: "§1.1 · pp. 3–8",
      minutes: 7,
      title: b("운반집합·생성자·초기 대수", "Carriers, constructors, and the initial-algebra view"),
      lead: b(
        "추상 구문은 특정 트리 구현이 아니라, 어떤 올바른 구현도 만족해야 하는 구조적 계약이다.",
        "Abstract syntax is a structural contract satisfied by any correct representation, not one privileged tree implementation."
      ),
      blocks: [
        prose(
          b(
            "구문을 진짜로 추상화하려면 ‘구절은 트리다’라고 특정 자료구조까지 고정해서는 안 된다. 대신 각 구절 종류에 운반집합(carrier)을 두고, 생산 규칙마다 운반집합 사이의 생성자 함수를 둔다. 이 장에는 변수 집합 `Var`, 정수식 집합 `IntExp`, 단언 집합 `Assert`가 있다. 예를 들어 변수 생성자는 `Cvar : Var → IntExp`, 덧셈 생성자는 `C+ : IntExp × IntExp → IntExp`, 전칭 수량자 생성자는 `C∀ : Var × Assert → Assert`의 형식을 가진다.",
          ...
          b(
            "상수처럼 자식이 없는 생성자는 빈 곱, 즉 원소가 하나뿐인 집합에서 운반집합으로 가는 함수로 볼 수 있다. `C0 : 1 → IntExp`라고 쓰면 `C0()`이 상수 0 구절이다. 구현 언어에서는 보통 `Const(0)`처럼 정수 하나를 받는 생성자로 모든 상수를 합치지만, 수학적 구조는 동일하다. 생성자의 인자 수와 인자 종류가 구절의 직접 부분구절을 정확히 정한다.",
        ...
        list(
          b("올바른 추상 구문 구현이 만족해야 할 세 조건", "Three conditions for a realization of abstract syntax"),
          b("단사성: 같은 생성자의 결과가 같으면 각 인자도 같아야 한다. `C+(e₀,e₁)=C+(e₂,e₃)`이면 `e₀=e₂`, `e₁=e₃`이다.", "Injectivity: equal results of one constructor imply equal corresponding arguments."),
          b("서로소 치역: 같은 운반집합으로 들어가는 서로 다른 생성자의 결과는 겹치지 않는다. 덧셈 구절이 동시에 곱셈 구절일 수 없다.", "Disjoint ranges: distinct constructors into one carrier never produce the same phrase."),
          b("유한 생성: 미리 주어진 집합의 원소를 제외한 모든 구절은 생성자를 유한 번 적용해 만들어져야 한다. 무한 깊이의 유령 구절은 허용하지 않는다.", "Finite generation: every non-predefined phrase is built by finitely many constructor applications; no infinitely deep phantom phrases are included.")
        ),
        prose(
          b(
            "첫 두 조건은 구절을 분해할 수 있게 한다. 어떤 단언이 `p₀ ∧ p₁` 꼴이라면 최상위 생성자가 논리곱이라는 사실과 두 직접 부분단언 `p₀`, `p₁`이 유일하다. 따라서 ‘구절의 모양에 따른 경우 분석’이 가능하다. 세 번째 조건은 이 분해를 반복하면 반드시 바닥의 상수나 변수에 도착함을 보장한다. 재귀 함수 정의와 구조적 귀납 증명이 바로 이 성질에 기대고 있다.",
          ...
          b(
            "유한 생성을 더 엄밀히 말하려면 깊이별 근사를 만든다. 깊이 0에는 아무 구절도 두지 않고, 깊이 `j+1`에는 깊이 `j` 이하의 구절들을 인자로 생성자를 한 번 적용해 얻는 구절을 넣는다. 모든 유한 깊이 집합의 합집합이 전체 운반집합이다. 이 정의는 2장에서 반복문의 의미를 근사하고 최소 고정점을 취하는 방식과 같은 모양을 갖는다. 구문 자체도 ‘생성 방정식의 최소 해’로 이해할 수 있다.",
        ...
        example(
          b("하나의 추상 구문, 여러 실현", "One abstract syntax, several realizations"),
          ...
          [
            ...
            b("구문 트리: 루트 `+`, 왼쪽 자식은 루트 `-binary`와 잎 0·1, 오른쪽 자식은 루트 `-unary`와 잎 2", "Syntax tree: root `+`; a binary-minus subtree over 0 and 1; a unary-minus subtree over 2."),
            b("대수적 자료형 값: `Plus(Sub(Const(0), Const(1)), Neg(Const(2)))`", "Algebraic-data-type value: `Plus(Sub(Const(0), Const(1)), Neg(Const(2)))`.")
          ],
          b(
            "표현은 다르지만 각 생성자의 단사성, 서로소 치역, 유한 생성이라는 같은 계약을 만족하므로 이후 의미 정의와 증명을 공유한다.",
        ...
        callout(
          ...
          b(
            "추상 구문은 생성자가 요구하는 것 외에는 아무것도 추가하지 않은 가장 자유로운 대수다. 생성자 방정식을 만족하는 어떤 목표 대수를 주어도, 각 구문을 그 대수의 값으로 보내는 구조 보존 함수가 정확히 하나 존재한다. 다음 절의 의미 함수가 유일하게 정해지는 이유를 이 한 문장이 압축한다.",
        ...
        callout(
          ...
          b(
            "그림으로 그린 트리가 추상 구문 그 자체라고 말하면 편리하지만 엄밀히는 트리도 하나의 실현이다. 핵심은 노드의 메모리 배치나 라벨 철자가 아니라 생성자를 유일하게 분해할 수 있는 구조다. 이 구분 덕분에 컴파일러 AST, 증명 보조기의 귀납 자료형, 종이 위의 생성자식이 같은 이론을 공유한다.",
      ...
      checkpoints: [
        b("생성자의 단사성과 서로소 치역이 각각 어떤 종류의 모호성을 막는지 구분해서 설명하라.", "Explain which ambiguity is prevented by constructor injectivity and which by disjoint ranges."),
    ...
    },
    {
      ...
      blocks: [
        prose(
          b(
            "닫힌 식 `2 + 3`은 상태와 무관하게 5를 낸다. 반면 `x + 3`의 값은 `x`가 무엇인지 알아야 정해진다. 그러므로 모든 정수식에 같은 종류의 의미를 주려면 의미를 `상태 → 정수` 함수로 잡아야 한다. 단언도 같은 이유로 `상태 → 진릿값` 함수가 된다. 상태 집합을 `Σ = Var → ℤ`, 진릿값 집합을 `𝔹 = {true,false}`라 두면 의미 함수의 형식이 선명해진다.",
          ...
          b(
            "표시적 의미론은 구문을 이미 이해한다고 가정하고 그 구문이 나타내는 수학적 대상을 지정한다. 이 장에서 정수식의 의미 대상은 `Σ → ℤ`, 단언의 의미 대상은 `Σ → 𝔹`이다. 의미 괄호 `⟦e⟧exp σ`는 상태 `σ`에서 식 `e`의 값을, `⟦p⟧assert σ`는 같은 상태에서 단언 `p`의 진릿값을 뜻한다. 괄호 안은 대상 언어, 괄호 밖 계산은 메타언어에 속한다.",
        ...
        list(
          b("구문 생성자를 따라가는 핵심 의미 방정식", "Core semantic equations following the constructors"),
        ...
        example(
          ...
          b(
            "계산은 언제나 최상위 생성자의 방정식을 선택하고 직접 부분구절로 내려간 뒤, 얻은 의미를 다시 조합하는 방식으로 진행된다.",
        ...
        example(
          ...
          [
            ...
            b("따라서 전체 단언은 원래 상태가 무엇이든 참이다.", "Therefore the entire assertion is true regardless of the original state.")
          ],
          b(
      ...
      checkpoints: [
        ...
        b("닫힌 단언의 의미가 상태와 무관해야 하는 이유를 직관적으로 설명하라.", "Explain intuitively why the denotation of a closed assertion must be independent of state."),
        b("정수식과 단언의 의미 함수 공역이 왜 다른가?", "Why do integer expressions and assertions have different semantic codomains?")
      ],
    },
    {
      ...
      lead: b(
        "변수의 의미는 철자만으로 정해지지 않는다. 어느 바인더가 그 발생을 지배하는지가 구문 구조의 일부다.",
      ...
      blocks: [
        ...
        example(
          ...
          b("단언 `∀x. (x<y ∨ ∃y. (x=y ∧ ∀x. x>y))`를 바깥에서 안쪽으로 읽는다.", "Analyze `∀x. (x<y ∨ ∃y. (x=y ∧ ∀x. x>y))` from outside inward."),
          [
          ...
          b("전체 단언의 자유 변수 집합은 `{y}`다.", "The free-variable set of the whole assertion is `{y}`.")
        ),
        prose(
          b(
            "자유 변수 집합 함수 `FV`는 구문을 따라 재귀적으로 정의한다. 상수와 진리 상수에는 자유 변수가 없고, 변수 `v`의 자유 변수는 `{v}`다. 단항 생성자는 자식의 집합을 그대로 전달하고, 이항 생성자는 두 집합의 합집합을 취한다. 수량자는 본문의 자유 변수 집합에서 자신이 묶는 이름 하나를 제거한다. 이 정의도 의미 정의는 아니지만 구문 지향적이므로 모든 구절에 유일한 결과를 준다.",
        ...
        callout(
          ...
          b(
            "구절 `p`의 자유 변수들에 대해 두 상태 `σ`, `σ'`가 같은 값을 준다면 `p`의 의미도 두 상태에서 같다. 식과 단언 모두에 대해 성립한다. 즉 의미 계산은 상태 전체를 받는 것처럼 보이지만 실제로는 유한 집합 `FV(p)`의 값만 관찰한다. 닫힌 구절은 자유 변수가 없으므로 어떤 두 상태에서도 의미가 같다.",
          ...
          b(
            "이 정리는 자유 변수의 정의가 단순한 구문 목록이 아니라 의미 의존성의 정확한 상한임을 보여 준다. 이후 최적화나 환경 축소에서 ‘이 구절이 읽을 수 있는 이름은 무엇인가’를 정당화하며, 함수 클로저가 어떤 환경을 보존해야 하는지 이해하는 바탕이 된다.",
        ...
        prose(
          b(
            "구조적 귀납법은 자연수 귀납법의 변형이 아니라 그 위에 정당화된다. 구절의 깊이를 자연수로 보고 직접 부분구절의 깊이가 항상 더 작다는 사실을 사용한다. 실무적으로는 ‘각 생성자 경우를 모두 다루고, 자식에 대해 명제를 가정한다’는 방식으로 쓴다. 생성자 하나를 빼먹으면 모든 구절을 덮지 못하고, 바인더 경우를 비바인더처럼 처리하면 상태 갱신 때문에 증명이 깨진다.",
      ...
      checkpoints: [
        ...
        b("일치 정리로 닫힌 단언의 상태 독립성을 한 줄로 유도하라.", "Derive state independence of a closed assertion from the Coincidence Theorem.")
      ],
    },
    {
      ...
      lead: b(
        "치환은 글자를 바꾸는 작업이 아니라 자유 변수의 의미 관계를 보존하는 구문 연산이다.",
      ...
      blocks: [
        prose(
          b(
            "`p`의 자유 변수 `v`에 식 `e`를 넣는다는 직관은 간단해 보인다. 하지만 `e`에 자유롭게 등장하는 이름이 `p` 안의 바인더와 같으면 단순 텍스트 교체가 그 이름을 새로 묶어 버린다. 이를 변수 포획(capture)이라 한다. 포획은 단지 이름이 보기 싫게 겹치는 문제가 아니라 결과 단언의 의미를 바꾼다.",
        ...
        example(
          ...
          [
            ...
            b("원래 의도는 현재 상태의 `y+1`보다 큰 정수가 존재한다는 말이어서 항상 참이지만, 포획된 결과는 어떤 정수도 자기보다 1 클 수 없다는 거짓 단언이다.", "The intended claim says some integer exceeds the state's current `y+1`, which is true; the captured result says some integer exceeds itself plus one, which is false."),
        ...
        prose(
          ...
          b(
            "상수는 그대로이고, 변수 `v`는 `S(v)`가 되며, 비바인딩 생성자에서는 자식들에 같은 치환을 재귀 적용한다. 어려운 절은 수량자다. `(∀v.p)/S`를 계산할 때 새 바인더 `v_new`를 고르되, 본문에서 자유롭게 남아 치환될 각 변수 `w`의 교체식 `S(w)`에 `v_new`가 자유롭게 나타나지 않게 해야 한다. 가능하면 기존 `v`를 유지하고, 충돌할 때만 표준 순서에서 처음 나오는 신선한 이름을 골라 정의 자체를 결정적으로 만든다.",
        ...
        callout(
          "key",
          b("치환의 세 구문 성질", "Three syntactic properties of substitution"),
          b(
        ...
        callout(
          ...
          b(
            "치환 정리는 구문 치환과 의미 상태 구성을 연결한다. 상태 `σ`에서 치환된 구절 `p/S`를 해석하는 것은, 각 원래 자유 변수 `w`의 값을 `σ'`에서 교체식 `S(w)`를 계산한 값으로 정한 상태에서 원래 `p`를 해석하는 것과 같다. 더 간단히 쓰면 새 상태 `σ_S(w)=⟦S(w)⟧σ'`에 대해 `⟦p/S⟧σ' = ⟦p⟧σ_S`다. 필요한 자유 변수에서만 두 상태가 일치하면 충분하다.",
          ...
          b(
            "증명은 다시 `p`에 대한 구조적 귀납이다. 비바인더 경우는 의미 방정식과 귀납가정으로 곧바로 끝난다. 수량자 경우에는 캡처 회피 조건이 결정적으로 쓰인다. 새 바인더 이름이 교체식들에 자유롭게 나타나지 않으므로 `σ'`에서 그 이름만 수량자 값 `n`으로 바꾸어도 교체식의 의미는 변하지 않는다. 그래서 구문 쪽 이름변경과 의미 쪽 원래 변수 상태 갱신이 정확히 맞물린다.",
        ...
        example(
          ...
          [
            b("구문 결과는 `(y+1)+2<y`다. 원래 `y`는 치환 대상이 아니므로 그대로 남는다.", "The syntactic result is `(y+1)+2<y`; the original `y` remains."),
        ...
        prose(
          ...
          b(
            "이름변경 정리는 바인더 이름을 본문에서 충돌하지 않는 새 이름으로 바꾸어도 의미가 같다고 말한다. `∀x.x+y>0`과 `∀z.z+y>0`은 같은 바인딩 구조를 갖는다. 이런 교체를 알파 변환(α-conversion)이라 한다. 더 나아가 고차 추상 구문 관점은 알파 변환으로 연결되는 표현들을 애초에 하나의 추상 구절로 본다. 바운드 이름의 철자는 의미 없는 구체 표기라는 생각이다.",
        ...
        callout(
          ...
          b(
            "대상 변수에 대상 구절을 넣는 치환은 포획 회피가 필요한 형식적 구문 연산이다. 메타언어 안에서 메타변수 식을 다른 메타언어 식으로 바꾸는 작업도 메타언어의 바인딩 규칙을 따른다. 반면 추론 규칙 스키마의 메타변수 `p`에 구체 대상 단언을 넣어 인스턴스를 만드는 것은 대상 언어 치환이 아니라 스키마 변수에 값을 정하는 일이다. 대상 변수와 메타변수를 섞으면 불필요한 이름변경을 하거나 실제 포획을 놓치게 된다.",
      ...
      checkpoints: [
        ...
        b("치환 정리의 양변에서 구문 조작과 상태 조작이 각각 어디에 나타나는가?", "Identify the syntactic operation and the semantic state operation on the two sides of the Substitution Theorem."),
    ...
    },
    {
      ...
      blocks: [
        example(
          b("연습 1 — 자연어를 수량자 구조로", "Practice 1 — Translate a counting statement"),
          b("‘0보다 크고 4보다 작은 서로 다른 정수가 적어도 두 개 있다’를 이 장의 단언으로 적어 보라.", "Express: ‘There are at least two distinct integers greater than zero and less than four.’"),
          [
        ...
        example(
          b("연습 4 — 새 바인딩 구문 설계", "Practice 4 — Design a new binding construct"),
          ...
          [
            b("추상 구문: `v`, 아래·위 경계식 `e₀,e₁`, 본문 `e₂`를 받는 생성자를 추가한다. `v`는 본문에서만 바인더이고 경계식에서는 자유로 읽는 설계를 명시한다.", "Add a constructor receiving `v,e₀,e₁,e₂`, specifying that `v` binds only in the body, not in the bounds."),
            ...
            b("그 뒤 일치 정리와 치환 정리의 새 생성자 경우를 증명해야 언어 확장이 기존 메타이론과 호환된다.", "Prove new cases of the Coincidence and Substitution Theorems to show compatibility with existing metatheory.")
          ],
        ...
        callout(
          ...
          b(
            "`(p/S)/S'`를 한 번의 치환 `p/S''`와 비교하려면 각 원래 자유 변수 `w`에서 `S''(w)`를 `(S(w))/S'`로 정의한다. 비바인더 생성자는 구조적 귀납으로 바로 맞아떨어진다. 바인더에서는 두 계산이 서로 다른 신선한 이름을 고를 수 있으므로 결과가 글자 그대로 같다고 기대하면 안 된다. 대신 이름변경 정리를 사용해 두 결과가 알파 동치임을 보이는 것이 정확한 결론이다.",
        ...
        list(
          ...
          b("구문적 동일성과 알파 동치를 구분했다.", "Literal syntactic equality was distinguished from alpha-equivalence.")
        )
      ],
      checkpoints: [
        b("위 연습을 보지 않고 약수와 소수의 단언을 다시 구성하라.", "Reconstruct the divisibility and primality assertions without looking back."),
        b("새 바인딩 구문을 추가할 때 함께 확장해야 하는 다섯 요소를 나열하라.", "List five components that must be extended when adding a new binding construct."),
    ...
    },
    {
      ...
      blocks: [
        list(
          ...
          b("먼저 표면 문자열에서 독립된 구절 종류와 생성자를 정한다. 단사성·서로소 치역·유한 생성이 재귀와 귀납의 토대를 만든다.", "Define phrase sorts and constructors independently of surface strings; injectivity, disjointness, and finite generation support recursion and induction."),
          b("각 구절 종류에 알맞은 의미 영역을 고르고, 모든 생성자에 구문 지향 의미 방정식을 하나씩 준다.", "Choose semantic domains and give one syntax-directed equation per constructor."),
          b("구문 지향성과 초기성으로 의미 함수의 존재·유일성과 합성성을 얻는다.", "Syntax direction and initiality give existence, uniqueness, and compositionality."),
          b("의미론으로 상태에서의 참과 타당성을 정의하고, 추론 규칙의 건전성으로 증명 가능성을 의미적 참에 연결한다.", "Use semantics to define truth and validity, then soundness to connect derivability with semantic truth."),
        ...
        prose(
          b(
            "이 흐름은 뒤에서 그대로 확대된다. 2장은 단언 대신 명령을 추가하고 의미 영역을 부분 상태 변환으로 바꾼다. 3장은 타당한 산술 단언을 이용해 프로그램 명세의 추론 규칙을 만든다. 10장에서는 수량자 대신 람다가 이름을 묶고 베타 축약이 치환을 요구한다. 11장에서는 구문 치환 대신 환경과 클로저가 같은 의미 효과를 구현한다. 타입 장에서는 ‘의미 값’ 대신 타입 판단을 생성자에 맞춘 규칙으로 유도한다.",
          ...
          b(
            "1장을 제대로 이해했다는 기준은 기호를 외우는 것이 아니다. 처음 보는 언어 기능을 만났을 때 ‘추상 생성자는 무엇인가, 의미 함수의 형식은 무엇인가, 각 생성자의 방정식은 부분 의미만 쓰는가, 어떤 이름을 묶는가, 치환 정리가 계속 성립하는가’를 스스로 묻는다면 이 장의 도구를 얻은 것이다.",
        ...
        callout(
          ...
          b(
            "논리식은 모든 상태에서 즉시 진릿값을 내는 전체 함수로 해석할 수 있었다. 하지만 `while true do skip` 같은 명령은 최종 상태를 내지 않는다. 구문 지향성과 합성성을 유지하면서 ‘결과가 없음’을 수학적 의미에 넣으려면 어떤 값과 어떤 순서 구조가 필요한가? 이 질문이 도메인과 최소 고정점으로 이어진다.",
      ...
      checkpoints: [
        b("처음 보는 `let v=e in p` 구문을 추가한다면 1장의 순서에 따라 무엇부터 무엇까지 정의할지 설계안을 써 보라.", "Design the additions needed for a new construct `let v=e in p`, following the chapter's workflow."),
